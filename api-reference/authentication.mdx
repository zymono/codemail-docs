...
<AccordionGroup>
  <Accordion title="Multiple Emails Per IP">
    We prevent to users from the same IP address from signing up in a row. (Another user has to signup to any Cdemail form on another IP to allow them to signup again)
